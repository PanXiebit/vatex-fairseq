...
from fairseq.data.video_language_dataset import VideoLanguageDataset
...
def load_videolang_dataset(data_path, split,
    ...
    shape_size, sample_startegy, tgtlen_times, transform, mean_img_file,
    ...
    def video_indexed_dataset(path):
        if dataset_impl == "raw":
            return indexed_dataset.VideoIndexedRawTextDataset(path)
        else:
            raise NotImplementedError("the dataset_impl of video language dataset must be raw.")

    def text_indexed_dataset(path, dictionary):
        if dataset_impl == "raw":
            return indexed_dataset.IndexedRawTextDataset(path, dictionary)
        else:
            raise NotImplementedError("the dataset_impl of video language dataset must be raw.")
    ...
    for k in itertools.count():
        ...
        src_datasets.append(video_indexed_dataset(prefix + src))
    ...
    return VideoLanguageDataset(
        ...
        shape_size=shape_size, sample_startegy=sample_startegy, tgtlen_times=tgtlen_times,
        transform=transform, mean_img_file=mean_img_file,
...
@register_task('videotranslation_lev')
class VideoTranslationLevenshteinTask(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            '--shape_size', default=224, type=int,
            help="the shape size of image")
        parser.add_argument(
            '--sample-startegy', default="sampling_with_tgt_len", type=str,
            choices=["sampling_with_tgt_len", "sampling_with_src_len"],
            help="sampling frames from the video"
        )
        parser.add_argument(
            '--tgtlen-times', default=5, type=int,
            help="The maximum number of frames of the source video is 'tgtlen_times' times"
                 "the length of the target sentence, if use, the sample_startegy must be "
                 "sampling_with_tgt_len")
        parser.add_argument(
            '--mean-img-file', default=None, type=str,
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_videolang_dataset(
            ...
            shape_size=self.args.shape_size,
            sample_startegy=self.args.sample_startegy,
            tgtlen_times=self.args.tgtlen_times,
            transform=None,
            mean_img_file=self.args.mean_img_file,
