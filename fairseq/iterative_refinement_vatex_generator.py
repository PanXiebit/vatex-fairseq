...
class IterativeRefinementGenerator(object):
    ...
    @torch.no_grad()
    def generate(self, models, sample, tgt_lang = "ch", prefix_tokens=None):
        ...
        bsz, src_len, num_frames = src_videos.size()
        
        tgt_lengths = sample["tgt_lengths"]
        print("gt tgt_lengths", tgt_lengths)

        # initialize
        encoder_out = model.forward_encoder([src_videos, src_lengths])
        langs, positions, prev_decoder_out = model.initialize_output_tokens(encoder_out, src_videos, tgt_lang)
        ...
        if self.beam_size > 1:
            ...
            length_beam_order = utils.new_arange(src_videos, self.beam_size, bsz).t().reshape(-1)
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, positions, langs, **decoder_options
            )
        ...
        if self.beam_size > 1:
            if reranker is not None:
                finalized = self.rerank(
                    reranker, finalized, [src_videos, src_lengths], self.beam_size
                )
