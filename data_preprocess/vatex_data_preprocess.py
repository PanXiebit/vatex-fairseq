...
if __name__ == "__main__":
    for split in ["train", "valid"]:
        ...
        with open(corpus, "r") as f:
            content = json.load(f)
            print(len(content))
        vid_paths = "Data/origin_data/{}.vid-ench.vid".format(split)
        ench_paths = "Data/origin_data/{}.vid-ench.ench".format(split)
        with open(vid_paths, "w") as fw1, open(ench_paths, "w") as fw2:
            for i, text in tqdm(enumerate(content)):
                ...
                for j in range(0, 5):
                    fw1.write(vid_path + "\n")
                    fw2.write("<en> " + tokenize(en_sents[j].lower()) + "\n")
                for j in range(0, 5):
                    fw1.write(vid_path + "\n")
                    fw2.write("<ch> " + tokenize(ch_sents[j]) + "\n")
                for j in range(5, 10):
                    fw1.write(vid_path + "\n")
                    fw2.write("<en> " + tokenize(en_sents[j].lower()) + " "
                              "<ch> " + tokenize(ch_sents[j]) + "\n")